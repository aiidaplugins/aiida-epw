...
def test_epw_restart_type_parameter(
    ...
    # 1. NONE: start from scratch and write the reusable EPW data.
    inputs_none = generate_inputs_epw(restart_type=RestartType.NONE)
    generate_calc_job(fixture_sandbox, "epw.epw", inputs_none)
    input_contents = Path(fixture_sandbox.abspath, "aiida.in").read_text()
    ...
    assert "epbwrite = .true." in input_contents
    assert "epbread = .false." in input_contents
    ...
    # 3. EPHREAD_RESTART
    inputs_read_restart = generate_inputs_epw(
        restart_type=RestartType.EPHREAD_RESTART,
        parameters={"INPUTEPW": {"scattering": True}},
        parent_folder_epw=generate_remote_data(fixture_localhost, "/remote/epw"),
    )
    generate_calc_job(fixture_sandbox, "epw.epw", inputs_read_restart)
    input_contents = Path(fixture_sandbox.abspath, "aiida.in").read_text()
    assert "epwread = .true." in input_contents
